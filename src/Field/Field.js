...
class Field {
  ...
  order() {
    if (arguments.length) {
      if (arguments[1] !== true) {
        console.warn(
          'Setting order with Field.order is deprecated, order directly in fields array'
        );
      }
  ...
  transform(fn) {
    ...
    if (typeof fn !== 'function') {
      let type = typeof fn;
      throw new Error(
        `transform argument should be a function, ${type} given.`
      );
    }
